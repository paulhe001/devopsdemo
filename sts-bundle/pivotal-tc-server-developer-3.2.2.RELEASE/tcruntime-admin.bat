...
rem ---------------------------------------------------------------------------
rem version: 3.2.2.RELEASE
rem build date: 20161101154337
...
"%JAVA_HOME%\bin\java" %JAVA_OPTS% -Dtcs.version=3.2.2.RELEASE -Dtcs.edition=developer -Djava.util.logging.config.file="%SCRIPT_DIR%\conf\commons-logging.properties" -Druntime.directory="%RUNTIME_DIR%" -Ddefault.instance.directory="%INSTANCE_DIR%" -classpath %CLASSPATH% com.springsource.tcruntime.instance.TcRuntimeAdmin %*
SET _RET=%ERRORLEVEL%
IF %_RET% EQU 1 (
    IF %ARG_COUNT% EQU 0 (
        pause
